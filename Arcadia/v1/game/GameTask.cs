    ...
    public class GameTask
    {
        ...
        public async Task<TaskQueuePacket> StartAsync(BaseSocketClient client, GameLobby lobby, GameDisplay display, GameClientData data, CancellationToken token)
        {
            ...
            async Task ParseAsync(SocketMessage message)
            {
                ...
                if (!(lobby.Receivers.Any(x => x.ChannelId == message.Channel.Id && x.State == GameState.Active) && lobby.TryGetUser(message.Author.Id, out Identity user)))
                {
                    ...
                    if (trigger.TryParse(new TaskTriggerContext(Data, player, message.Content), out GameTriggerResult context))
                    {
                        display.UpdateWindow(GameState.Active,
                            new ElementUpdatePacket(
                                new Element($"[Console] Trigger successful. (trigger:{trigger.Name})"),
                ...
                if (LastAttributesUpdated.Count == 0)
                    display.UpdateWindow(GameState.Active,
                        new ElementUpdatePacket(new Element($"[{user.Name}]: {message.Content}"),
                        ElementUpdateMethod.AddToGroup,
                        groupId: "elements.chat"));
                ...
                if (LastAttributesUpdated.Count > 0)
                    ...
                    x.AttributeCriteria.Any(y => LastAttributesUpdated.Select(z => z.Id).Contains(y.RequiredId))))
                        if (criteria.Check(Attributes))
                        {
                            display.UpdateWindow(GameState.Active,
                                new ElementUpdatePacket(
                                    new Element($"[Console] Task closed. ({Id} => {criteria.OnSuccess.NextTaskId})"),
                                    ElementUpdateMethod.AddToGroup,
                                    groupId: "elements.chat"));
