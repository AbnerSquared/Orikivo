...
namespace Arcadia.Old
{
    ...
    public class Game
    {
        ...
        private async Task OnOffhandMessageAsync(SocketMessage message)
        {
            ...
            if (!(Receivers.Any(x => x.ChannelId == message.Channel.Id && x.State == GameState.Inactive)
                && ContainsUser(message.Author.Id)))
                return;
        ...
        private async Task OnDisplayUpdatedAsync(GameDisplay display)
        {
            ...
            Receivers.ForEach(async x =>
            {
                ...
                Console.WriteLine($"-- ({x.Id}.{x.State}) Display updated. --");
            });
        }
        ...
        internal async Task StartSessionAsync()
        {
            while (UseMainHandler)
            {
                ...
                {
                    if (!(Receivers.Any(x => x.ChannelId == message.Channel.Id && x.State == GameState.Inactive)
                        && Lobby.TryGetUser(message.Author.Id, out Identity user)))
                    {
                        return;
                    }
                    ...
                    if (message.Content == "start")
                    {
                        Display.UpdateWindow(
                            GameState.Inactive,
                            new ElementUpdatePacket(
                                new Element($"[Console] {Lobby.Mode} will be starting soon."),
                                ElementUpdateMethod.AddToGroup,
                                groupId: "elements.console"));
                    ...
                    else
                    {
                        Display.UpdateWindow(
                            GameState.Inactive,
                            new ElementUpdatePacket(
                                new Element($"[{user.Name}]: {message.Content}"),
                                ElementUpdateMethod.AddToGroup,
                                groupId: "elements.console"));
                    }
                ...
                // sets the actual listener and stops it upon either start or close was called
        ...
        public async Task<GameResult> StartAsync()
        {
            ...
            Display.UpdateWindow(GameState.Active,
                new ElementUpdatePacket(ElementUpdateMethod.ClearGroup,
                groupId: "elements.chat"));

            Display.UpdateWindow(GameState.Inactive,
                new ElementUpdatePacket(new Element("[Console] The game has ended."),
                ElementUpdateMethod.AddToGroup,
                groupId: "elements.console"));
