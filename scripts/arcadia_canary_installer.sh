...
while [ $choice -eq 6 ]; do
	...
	if [ $choice -eq 1 ]; then
		...
		wget -N https://raw.githubusercontent.com/AbnerSquared/Orikivo/v1.0.0rc-1/scripts/arcadia_install_preq.sh && bash "$root/arcadia_install_preq.sh"
		bash "$root/arcadia_linux_install.sh"
	else
		if [ $choice -eq 2 ]; then
			echo "Downloading Arcadia. Please wait..."
			wget -N https://raw.githubusercontent.com/AbnerSquared/Orikivo/v1.0.0rc-1/scripts/arcadia_canary_updater.sh && bash "$root/arcadia_canary_updater.sh"
			bash "$root/arcadia_linux_install.sh"
		else
			if [ $choice -eq 3 ]; then
				...
				if [ $choice -eq 4 ]; then
					...
					wget -N https://raw.githubusercontent.com/AbnerSquared/Orikivo/v1.0.0rc-1/scripts/arcadia_run.sh && bash "$root/arcadia_run.sh"
