...
namespace Orikivo
{
    ...
    public class TimeCycle
    {
        ...
        public static GammaColorMap SunrisePalette = new GammaColorMap(0x15123D, 0x261948, 0x381B51, 0x4F285D, 0x653366, 0x894B78, 0xA96786, 0xC5748D);
        public static GammaColorMap MeridianPalette = new GammaColorMap(0x004F99, 0x0070B1, 0x008DC3, 0x2AAFDB, 0x5ED5ED, 0x7FE3EF, 0x99EDF5, 0xDBFCFC);
        public static GammaColorMap DuskPalette = new GammaColorMap(0x681467, 0x7F1A70, 0xA21E7B, 0xDB308E, 0xE95490, 0xEF6C94, 0xF87C90, 0xF59098);
        public static GammaColorMap NightPalette = new GammaColorMap(0x020A21, 0x091934, 0x0D213D, 0x00314F, 0x194A5A, 0x346061, 0x397773, 0x2E8982);
        ...
        public static GammaColorMap FromHour(float hour) // TODO: Incorporate offsets, and incorporate Range.Markers (when ready).
        {
            ...
            if (hour > cycle.NightEnd && hour <= cycle.Dawn)
                return GammaColorMap.Merge(NightPalette, SunrisePalette, GetHourStrength(cycle.NightEnd, cycle.Dawn, hour));

            if (hour > cycle.Dawn && hour <= cycle.Sunrise)
                return GammaColorMap.Merge(SunrisePalette, DuskPalette, GetHourStrength(cycle.Dawn, cycle.Sunrise, hour));

            if (hour > cycle.Sunrise && hour <= cycle.Meridian)
                return GammaColorMap.Merge(DuskPalette, MeridianPalette, GetHourStrength(cycle.Sunrise, cycle.Meridian, hour));

            if (hour > cycle.Meridian && hour <= cycle.Sunset)
                return GammaColorMap.Merge(MeridianPalette, DuskPalette, GetHourStrength(cycle.Meridian, cycle.Sunset, hour));

            if (hour > cycle.Sunset && hour <= cycle.Dusk)
                return GammaColorMap.Merge(DuskPalette, SunrisePalette, GetHourStrength(cycle.Sunset, cycle.Dusk, hour));

            if (hour > cycle.Dusk && hour <= cycle.NightStart)
                return GammaColorMap.Merge(SunrisePalette, NightPalette, GetHourStrength(cycle.Dusk, cycle.NightStart, hour));
