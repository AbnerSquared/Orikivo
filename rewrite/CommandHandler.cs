...
namespace Orikivo
{
    ...
    public class CommandHandler
    {
        ...
        private async Task UpdateAsync(CommandInfo command, OriCommandContext ctx)
        {
            ...
            else if (!OriJsonHandler.JsonExists<User>(ctx.User.Id))
            {
                if (ctx.Account == null)
            ...
            else if (!OriJsonHandler.JsonExists<OriGuild>(ctx.Guild.Id))
            {
                if (ctx.Guild == null)
            ...
            OriJsonHandler.Save(ctx.Container.Global, "global.json");
