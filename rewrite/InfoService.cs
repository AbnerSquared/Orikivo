    ...
    public class InfoService
    {
        ...
        public ContextResult Search(ContextInfo ctx)
        {
            ...
            if (ctx.HasRoot)
            {
                ...
                if (Checks.NotNull(_guild))
                {
                    if (module == null && group == null && Checks.NotNullOrEmpty(_guild.Options.Commands))
                    {
                        if (_guild.Options.Commands.Any(x => x.Name == ctx.Root))
                            return ContextResult.FromSuccess(GetMainCustom(ctx.Root));
                    }
                }
        ...
        public ModuleInfo GetMainModule(string moduleName, ModuleInfo parent = null)
        {
            ...
            if (!Checks.NotNullOrEmpty(modules))
                throw new NullResultException($"No matches were found{(Checks.NotNull(parent) ? $" within {parent.Name}" : "")} when searching for modules with the name '{moduleName}'.");
        ...
        public ModuleInfo GetMainGroup(string groupName, ModuleInfo parent = null)
        {
            ...
            if (!Checks.NotNullOrEmpty(groups))
                throw new NullResultException($"No matches were found{(Checks.NotNull(parent) ? $" within {parent.Name}" : "")} when searching for groups of the name '{groupName}'.");
        ...
        public ContextValue GetMainValue(string valueName, ModuleInfo parent = null)
        {
            ...
            if (!Checks.NotNullOrEmpty(values))
                throw new NullResultException($"No matches were found{(Checks.NotNull(parent) ? $" within {parent.Name}" : "")} when searching for a value with the name '{valueName}'.");
            ...
            if (values.Where(x => x.Type.EqualsAny(ContextInfoType.Module, ContextInfoType.Group)).Count() == 1)
