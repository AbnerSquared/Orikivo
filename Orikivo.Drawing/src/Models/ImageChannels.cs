    ...
    public class ImageChannels
    {
        ...
        public static ImageChannels FromImage(Bitmap image)
        {
           Grid<Color> pixels = GraphicsUtils.GetPixels(image);
            ...
            channels.R.SetEachValue((int x, int y, float z) => RangeF.Convert(0, 255, 0.0f, 1.0f, pixels.GetValue(x, y).R));
            channels.G.SetEachValue((int x, int y, float z) => RangeF.Convert(0, 255, 0.0f, 1.0f, pixels.GetValue(x, y).G));
            channels.B.SetEachValue((int x, int y, float z) => RangeF.Convert(0, 255, 0.0f, 1.0f, pixels.GetValue(x, y).B));
