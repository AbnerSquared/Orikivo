...
namespace Orikivo
{
    public static class EnumerableExtensions
    {
        ...
        public static IEnumerable<T> Shift<T>(this IEnumerable<T> enumerable, int shift)
        {
            ...
            int sourceIndex = direction > 0 ? 0 : direction * shift;
            int destinationIndex = direction > 0 ? shift : 0;
            int length = array.Length - Math.Abs(shift);

            Array.Copy(array, sourceIndex, array, destinationIndex, length);

            int clearIndex = direction > 0 ? 0 : array.Length - (direction * shift);

            Array.Clear(array, clearIndex, Math.Abs(shift));
