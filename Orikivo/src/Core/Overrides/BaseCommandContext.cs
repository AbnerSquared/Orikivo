...
namespace Orikivo
{
    public abstract class BaseCommandContext<TContainer, TGuild, TUser> : SocketCommandContext
        where TGuild : BaseGuild
        where TUser : BaseUser
        where TContainer : BaseContainer<TGuild, TUser>
    {
        protected BaseCommandContext(DiscordSocketClient client, TContainer data, SocketUserMessage message)
            : base(client, message)
        {
            Data = data;
        }
