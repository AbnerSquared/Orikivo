...
namespace Orikivo
{
    public class ShopHandler : MatchAction
    {
        ...
        public IUserMessage Initial { get; private set; }
        ...
        private string GetBackpackContent()
        {
            ...
            if (Husk.Backpack.ItemIds.Count == 0)
                ...
                bp.AppendJoin("\n", Husk.Backpack.ItemIds.Select(x => GetItemSummary(Engine.GetItem(x.Key), x.Value, true)));
        ...
        public override async Task<ActionResult> InvokeAsync(SocketMessage message)
        {
            ...
            if (State == MarketState.SellComplete)
            {
                ...
                {
                    ulong balance = Context.Account.Balance;
                    Last.ApplyBuy(Context.Husk, Context.Brain, ref balance);
                    Context.Account.Balance = balance;
