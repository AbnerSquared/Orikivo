...
namespace Orikivo
{
    public static class ChannelExtensions
    {
        ...
        public static async Task<RestUserMessage> ReplaceAsync(this RestUserMessage message, string filePath,
            ...
            RequestOptions options = null, bool isSpoiler = false)
        {
            RestUserMessage next = await ((SocketTextChannel)message.Channel).SendFileAsync(filePath, Check.NotNull(text) ?
                text : message.Content, isTTS, Check.NotNull(embed) ?
                embed : message.Embeds.FirstOrDefault(x => x.Type == EmbedType.Rich), options);
        ...
        {
            RestUserMessage next = await ((SocketTextChannel)message.Channel).SendMessageAsync(Check.NotNull(text) ?
                text : message.Content, isTTS, Check.NotNull(embed) ?
                embed : message.Embeds.FirstOrDefault(x => x.Type == EmbedType.Rich), options);
