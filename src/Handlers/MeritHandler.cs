...
namespace Orikivo
{
    ...
    internal static class MeritHandler
    {
        ...
        internal static string ViewDefault(User user)
        {
            ...
            {
                IEnumerable<string> merits = WorldEngine.Merits.Where(x => x.Value.Group == type).Select(x => x.Key);
        ...
        internal static string ViewCategory(User user, MeritGroup group)
        {
            ...
            if (WorldEngine.GetMerits(group).Count() == 0)
            ...
            foreach (Merit merit in WorldEngine.GetMerits(group))
            {
        ...
        internal static bool Claim(User user, string meritId)
        {
            ...
            {
                Merit merit = WorldEngine.GetMerit(meritId);
        ...
        {
            if (!WorldEngine.Merits.ContainsKey(meritId))
        ...
        private static bool Exists(User user, string meritId)
            => WorldEngine.Merits.ContainsKey(meritId) && user.HasMerit(meritId);

        private static bool ItemExists(string itemId)
            => WorldEngine.Items.ContainsKey(itemId);
